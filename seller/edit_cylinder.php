...
<section class="container-fluid container">
	...
	$Oxygen = new Oxygen();
	 ...
	 <?php
		if(isset($_GET['cylinder_id'])){
			$result = $Oxygen->singleOxygen($_GET['cylinder_id']); 
	...
	<?php
		if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['addpharmacy'])){
		   $cutomerlog = $Oxygen->updateOxygen($_POST, $_GET['cylinder_id']);
	...
	<div class="row p-3">
		<div class="col-md-4">
			<h2 class="text-center text-primary p-4"> Edit Oxygen</h2>
			<hr>
			<div>
				<form class="d-flex flex-column" action="" method="post" enctype="multipart/form-data">
					<div>
						<label><b>Cylinder name :</b></label>
						<input class="form-control" type="text" name="name" value="<?php echo $value1['name'] ?>" />
					</div>
					<div>
...
<input class="form-control" type="text" name="price" value="<?php echo $value1['cylinder_price'] ?>" />
					</div>	
